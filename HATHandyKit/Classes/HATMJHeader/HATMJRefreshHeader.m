...
- (void)prepare
{
    ...
    // 设置控件的高度
    self.mj_h = 55;
    self.backgroundColor = [UIColor colorWithHexString:@"0xEFEFF4"];
    
    UIView *backgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreen_Width, kScreen_Height)];
    backgroundView.backgroundColor = [UIColor colorWithHexString:@"0xEFEFF4"];
...
- (void)placeSubviews
{
    ...
    self.backgroundView.center = CGPointMake(self.mj_w * 0.5, self.mj_h - self.backgroundView.mj_h * 0.5);
}
...
- (void)setUpdateImage:(UIImage *)updateImage {
    self.loading.image = updateImage;
}
