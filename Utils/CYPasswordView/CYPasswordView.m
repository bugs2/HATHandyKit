...
//    self.imgRotation.centerY = self.passwordInputView.height * 0.5;
    ...
    self.imgRotation.centerY = CYPasswordViewTitleHeight + CYPasswordViewTextFieldMarginTop + CYPasswordViewTextFieldHeight * 0.5;
...
- (void)showInView:(UIView *)view
{
    ...
    //计算高度
    //head(44) + pad(20) + textfield(?) + pad(20) + forgetPassword(20) + pad(20) + keyboard(216/226)
    CYPasswordViewTitleHeight = 44;
    CYPasswordViewAnimationDuration = 0.25;
    CYPasswordViewCloseButtonWH = 44;
    CYPasswordViewCloseButtonMarginLeft = 0;
    CYPasswordViewTextFieldWidth = kScreen_Width - 30;
    CYPasswordViewTextFieldHeight = CYPasswordViewTextFieldWidth / 6;
    CYPasswordViewTextFieldMarginTop = 20;
    CYPasswordViewForgetPWDButtonMarginTop = 12;
    
    if (kDevice_Is_iPhone6Plus) {
        CYPasswordViewPointnWH = 12;
        CYPasswordKeyboardHeight = 226;
    } else if (kDevice_Is_iPhone6) {
        CYPasswordViewPointnWH = 10;
        CYPasswordKeyboardHeight = 216;
    } else {
        CYPasswordViewPointnWH = 10;
        CYPasswordKeyboardHeight = 216;
    }
    
    CYPasswordInputViewHeight = CYPasswordViewTitleHeight + CYPasswordViewTextFieldMarginTop + CYPasswordViewTextFieldHeight + CYPasswordViewForgetPWDButtonMarginTop + 20 + 20 + CYPasswordKeyboardHeight;
...
- (void)requestComplete:(BOOL)state {
    if (state) {
        [self stopLoading];
        self.passwordInputView.titleColor = kColor333;
//        [self hide];
    } else {
        [self stopLoading];
//        [self showKeyboard];
        //错误字体颜色切换
        self.passwordInputView.titleColor = kColorBrandRed;
        //清空密码
        NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionary];
        userInfoDict[CYPasswordViewKeyboardNumberKey] = @"";
        [CYNotificationCenter postNotificationName:CYPasswordViewNumberButtonClickNotification object:self userInfo:userInfoDict];
    }
}
