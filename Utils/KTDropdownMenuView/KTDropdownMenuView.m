...
static const CGFloat kKTDropdownMenuViewAutoHideHeight = 100;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *cellText = [NSString stringWithFormat:@"%@  %@", title, content];
    NSMutableAttributedString *attriCellText = [[NSMutableAttributedString alloc] initWithString:cellText];
    [attriCellText addAttribute:NSForegroundColorAttributeName value:self.cellTitleColor range:NSMakeRange(0,title.length)];
    //多加2个空格
    [attriCellText addAttribute:NSForegroundColorAttributeName value:self.cellContentColor range:NSMakeRange(title.length + 2, content.length)];
    
    cell.textLabel.attributedText = attriCellText;
...
- (void)hideMenu
{
    ...
    CGFloat tableCellsHeight = _cellHeight * _titles.count;
    [self.tableView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.wrapperView.mas_top).offset(-tableCellsHeight - kKTDropdownMenuViewHeaderHeight - 10);
        make.bottom.equalTo(self.wrapperView.mas_bottom).offset(tableCellsHeight + kKTDropdownMenuViewHeaderHeight + 10);
    }];
...
- (UIColor *)cellTitleColor {
    if (!_cellTitleColor)
    {
        _cellTitleColor = [UIColor whiteColor];
    }
...
- (UIColor *)cellContentColor {
    if (!_cellContentColor)
    {
        _cellContentColor = [UIColor whiteColor];
    }
    
    return _cellContentColor;
}
...
- (void)setBUserInteractionEnabled:(BOOL)isUserInteractionEnabled {
    self.titleButton.userInteractionEnabled = isUserInteractionEnabled;
    self.arrowImageView.hidden = !isUserInteractionEnabled;
}
